...
// ─── Admin stacked sections ────────────────────────────────────────────────────

function AdminSubAdminSections() {
  return (
    <>
      <SubAdminSection
        title="Aggregators"
        role="aggregator"
        queryKey={adminQueryKeys.users.byRole("aggregator")}
        queryFn={() => fetchAdminUsers("aggregator")}
        headerAction={
          <AdminInviteButton
      ...
      <SubAdminSection
        title="Logistics"
        role="logistics"
        queryKey={adminQueryKeys.users.byRole("logistics")}
        queryFn={() => fetchAdminUsers("logistics")}
        headerAction={
          <AdminInviteButton
...
function AdminInviteButton({ defaultRole }: { defaultRole: "aggregator" | "logistics" }) {
  const queryClient = useQueryClient()
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-5 pb-6">
        <SubAdminInfoBanner />
        {isSuperAdmin ? <SuperAdminSubAdminTabs /> : <AdminSubAdminSections />}
